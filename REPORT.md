...
2. **5대 영역 탭 구성**:
   ...
   - **Tab 4. 30일 예측 시뮬레이션**: **ARIMA (자기회귀 이동평균)** 및 **Holt 지수평활법** 기반 고도화 시계열 예측, 모델별 과거 30일 **백테스팅 오차율(MAPE %, RMSE)** 비교 표 제공, 다중 모델 추세선 동시 비교 및 예측 데이터 CSV 다운로드
...
- **예측 모델 고도화 및 2024년 훈련 ➔ 2025년 주가 검증**:
  - 2024년 1년간 데이터(244일)만으로 모델을 학습시킨 후 2025년 전체 영업일 주가(241일)를 예측하여 실제 주가와 대조 검증했습니다.
  - **2025년 상반기 (6개월)**: ARIMA 모델 오차율 **MAPE 5.84% (94.16% 예측 정확도)** 기록.
  - **2025년 전체 1년**: ARIMA 모델 오차율 **MAPE 21.34%**, MAE 18,491.7원을 기록하며, OLS 선형회귀(37.02% MAPE) 대비 오차율을 **42% 이상 감축**하였습니다.
  - 검증 시각화 차트: ![2024년 학습 기반 2025년 주가 예측 검증](images/05_2024_train_2025_test_validation.png)

### 한계점
1. **외부 독립 변수 미반영**: 본 분석은 주가/거래량 단일 시계열 데이터에 한정되어, DRAM/NAND 가격, 환율(USD/KRW), 미국 반도체 지수(SOXX) 등 거시 변수를 직접 입력값으로 사용하지 않았습니다.
2. **시계열 고도화 모델의 한계**: ARIMA 및 지수평활 모델은 시계열 자체의 오차와 자기상관을 충실히 학습하지만, 실적 발표나 돌발적 정치/금리 뉴스에 의한 급격한 점프(Discontinuity)는 사전에 예지하지 못하는 제약이 있습니다.
